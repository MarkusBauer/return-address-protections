        ...
        if (prolog.savedRegisters[i].location !=
          ...
          if (registers.validFloatRegister(i))
            ...
            returnAddress = getSavedRegister(addressSpace, registers, cfa,
                                             prolog.savedRegisters[i]);
            // PATCH MOD START HERE
            // BEGIN RIP Xoring - check if address has been xored, and restore it if needed
            // Un-xored addresses are typically 0x7f?????????? or 0x4?????
            // In every case, the highest 2 bytes (17 bits) are 0.
            if (returnAddress >> 47){
              uintptr_t secret;
              __asm__("movq %%fs:0x28, %0;" : "=r"(secret));
              // __asm__("movq $0x10000000000000, %0;" : "=r"(secret)); // DEBUG constant
              returnAddress ^= secret;
            }
            // END RIP Xoring
          }
